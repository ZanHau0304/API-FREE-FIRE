/////////////////////////////////////////////////////////////////////
//    MODULES MẪU SỬ DỤNG API KEY CỦA TÍNH ĐIỂM LOGO + TÊN TEAM    //
/////////////////////////////////////////////////////////////////////
...
function saveBase64ToFile(base64, prefix = "tdlg") {
...
function buildTinhDiemUrl(opts) {
  const {
    ...
    ct, ct2, logo, xoatran, cpr, TeamName, LogoTeam, data
  } = opts;
...
async function callTinhDiemLogo({
  api, threadID, messageID,
  idgame, keyName, keyInfo,
  start, end, xoa, cpr, dataUid
}) {
  ...
  try {
    ...
    const imgFile = saveBase64ToFile(res.data.base, "tdlg");
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!args || !args[0]) {
    return api.sendMessage(
      "Cách dùng:\n" +
      ".tinhdiemlogo <id> [key] [xoa1,3] [cpr50] (rút gọn -> chọn khung giờ)\n" +
      ".tinhdiemlogo <id> <start DD/MM/YYYY> <startTime> <end DD/MM/YYYY> <endTime> [key] [xoa..] [cpr..]\n" +
      "Ví dụ:\n.tinhdiemlogo 123456789 fftest xoa1,3 cpr50\n.tinhdiemlogo 123456789 01/01/2026 13:00 01/01/2026 15:00 fftest\n",
      threadID, messageID
    );
  }
  ...
  if (fullDetected) {
    ...
    const dataUid = String(senderID);

    return callTinhDiemLogo({
      api, threadID, messageID,
      idgame, keyName, keyInfo,
      start: startMoment,
      end: endMoment,
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  if (handleReply.type !== "tdlg_select_slot") return;
  ...
  return callTinhDiemLogo({
    ...
    dataUid: handleReply.data.dataUid
  });
};
